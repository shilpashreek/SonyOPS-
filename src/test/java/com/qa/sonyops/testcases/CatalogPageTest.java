...
public class CatalogPageTest extends BaseClass {
	...
	public void ValidateResultsAreDisplayedAccToAppliedFilters() throws InterruptedException {
		...
		Thread.sleep(2000);
		System.out.println("Page Count initially" + " " + PageCountInitially + " "
				+ "page count after performing search" + " " + " " + PageCountAfterSearch + " "
				+ "Pagecount after applying filter" + " " + PageCountAfterCatalogStatus);
		Assert.assertTrue(PageCountAfterSearch_int < PageCountInitially_int
				&& PageCountAfterCatalogStatus_int < PageCountAfterSearch_int, "Filters are not applied");
