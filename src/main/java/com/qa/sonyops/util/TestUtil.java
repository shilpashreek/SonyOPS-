...
public class TestUtil extends BaseClass {
	...
	public static String TESTDATA_SHEET_PATH = "D:\\automationPULLprojects\\SonyOPS-\\src\\main\\java\\com\\qa\\sonyops\\testdata\\SonyopsTestData.xlsx";
	FileInputStream fis;
	static Workbook wb;
	Sheet sheet;
...
//method to wait till invisibility of initialising 
	public static void Initialsising(WebDriver driver, int timeout) {
		new WebDriverWait(driver, timeout).until(ExpectedConditions.invisibilityOfElementLocated(
				By.cssSelector("span#CMP_CatalogPlayerV2Container_playerStateDisplayText")));
...
//method to get total duration of the video
	public String GetTotalDurationOfVideo() {
		String EndTime = "null";
		try {
			WebElement VideoDuration = driver
					.findElement(By.cssSelector("span#CMP_SearchPlayerContainer_totalDuration"));
	...
	public boolean TimecodeComparison(String TC_IN_value, String TC_OUT_value) {
		...
		for (int i = 2; i < beforePlay_TimeValue.length; i++) {
			if (!beforePlay_TimeValue[i].equals(afterPlay_TimeValue[i])) {
				System.out.println("Video start_time is" + " " + beforePlay_TimeValue[i] + " " + "and" + " "
						+ "End_time is" + " " + afterPlay_TimeValue[i]);
				...
				System.out.println(
						"Asset is not streaming" + " " + beforePlay_TimeValue[i] + " " + afterPlay_TimeValue[i]);
...
//Method to TakeScreenshot in case of exception
	public static void ScreenShotOnException() throws IOException {
		...
		FileUtils.copyFile(src,
				new File("D:/automationPULLprojects/SonyOPS-/screenshots" + System.currentTimeMillis() + "_" + ".png"));
